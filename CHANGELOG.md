...
## 0.577 — North-Star Phases 2–3: honest verdicts, current anchors, a checkable protocol

- **Truth ledger schema 2 — CONFIRMED must be earned, never asserted.** A field note from the LineCheck reviewer (a peer AI session) showed `forge ledger append --verdict CONFIRMED` accepted a hand-asserted verdict with no binder re-deriving anything — a HEALTHY hash chain of unverified claims. `CONFIRMED` is now reserved for binder-derived verdicts that carry a re-runnable reproduce command and a ground-truth binding; an unbound human/model judgement is **`ATTESTED`**, never silently rendered as CONFIRMED. Enforced at write (`append_claim` refuses an unbound CONFIRMED) and at read (`verify_ledger` flags a smuggled or legacy one → BLOCKED). This is "never upgrade NOT_RUN to PASS" applied to the ledger itself.
- **Head-integrity guard (R8) — no false CONTRADICTED at a stale anchor.** The `forge bind` binders gated on object *existence*, which a rebased/force-pushed-away head or a stale remote-tracking snapshot both satisfy — so a binder could contradict a claim using yesterday's tree. A shared guard (`core/ref_integrity`) now classifies every anchor first: a missing, non-commit, superseded, or stale/unverifiable remote head yields NOT_RUN (→ terminal UNVERIFIABLE in the ledger), never a content verdict. Remote-tracking names are verified against the remote *now* via `ls-remote`.
- **Generalized structured-claim binder (R3) — `forge bind claims`.** head-equals, ancestry, blob-identity, file-sha256, and file-regex claims, each evaluated at guarded anchors with its reproduce command; `--ledger` records the verdict as a binder-derived ledger entry.
- **`forge protocol verify` (R9) — a multi-agent collaboration's invariants, machine-checked.** Exact-head claims (head currency via the R8 guard), bindable receipts, structured (not prose-only) state with unique ids, history-preserving supersession, owner-gated irreversible acts, and a single-baton `--liveness` check (exactly one conserved holder, else STALLED). Verdicts feed the truth ledger.
- Restores a green runtime-reachability gate that the 0.576 merge had reddened (an unclassified merged-in coordination lane). An adversarial multi-agent review of the R8/R3 work caught and fixed seven correctness defects before commit. Narrative integrity PASS; self-test green. Release authorization remains **false**.

## 0.576 — G1 truth-anchoring binders + the witness truth ledger

- **`forge bind` — three deterministic G1 binders (advisory, read-only).** Source-anchored release verification binds accepted-vs-candidate release truth to the accepted *source commit's* code, not to RELEASE-STATE claims. The gate-coverage differ reports checks that exist in a tree but are not invoked by its CI gate (each unwired check NOT_RUN), so a green gate cannot read as "covered" while silently omitting assertions. Gate-diff monotonicity certifies that a change to a project's gate/check scripts removed no assertion and introduced no SKIP path into existing checks. Each verdict maps to an exit code so the command doubles as a CI gate. All three were delivered from real misses surfaced while observing a live cross-model collaboration, and the gate-coverage finding was adopted in the field.
- **`forge ledger` — an append-only, hash-chained witness truth ledger.** Records a claim bound to ground truth with a verdict (CONFIRMED / CONTRADICTED / UNVERIFIABLE / INCOMPLETE). Supersede-not-delete preserves lineage; a changed verdict is a new linked entry and the prior one stays readable; verdict flips are surfaced, never erased; no path auto-upgrades a verdict — UNVERIFIABLE is terminal (the ledger analog of "never upgrade NOT_RUN to PASS"); any tamper is caught by the chain hash. First increment of the North-Star pivot: Forge as the machinery that maintains a truthful, evolving model of a subject across sessions.
- New modules classified under G1 in the runtime-reachability map; report regenerated. Narrative integrity PASS; self-test green. Release authorization remains false.
